...
int main()
{
    ...
    LayoutVertexData<VertexLayout::PositionTexcoord> windowVertexData(sizeof(float) * gTransparentWindowVertices.size(), gTransparentWindowVertices.data());
    ...
    Texture windowTexture("./resource/window.png", 5);
    ...
    std::vector<uptr<Object>> transparentObjects;
    for (int i = 0; i < gTransparentWindowPosition.size(); i++)
    {
        uptr<Object> pCube = std::make_unique<Object>(&windowVertexData, &windowShader, &windowTexture);
        pCube->SetPosition(gTransparentWindowPosition[i]);
        pCube->SetRotation(glm::quat_cast(glm::rotate(glm::mat4(1.0f), glm::radians(45.0f), glm::vec3(0.0f, 1.0f, 0.0f))));

        transparentObjects.push_back(std::move(pCube));
    }
...
#pragma region [FORWARD PASS] Render light cube & trnasparent
        ...
        std::sort(transparentObjects.begin(), transparentObjects.end(), [&camera](const uptr<Object>& left, const uptr<Object>& right) -> bool
            {
                float distanceLeft = glm::length(camera.GetPosition() - left->GetPosition());
                float distanceRight = glm::length(camera.GetPosition() - right->GetPosition());
                return distanceLeft > distanceRight;
            });

        for (auto& pRenderObject : transparentObjects)
        {
            // 1. Bind vertex attribute
            ::glBindVertexArray(pRenderObject->GetVertexData()->GetVertexArrayHandle());
